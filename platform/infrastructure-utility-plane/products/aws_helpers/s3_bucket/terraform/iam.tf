data "aws_iam_policy_document" "bucket_read_policy" {
  statement {
    sid    = "AllowBucketRead${replace(var.bucket_name, "-", "")}"
    effect = "Allow"
    actions = [
      "s3:ListBucket",
      "s3:GetBucketAcl",
  ...
  dynamic "statement" {
    ...
    content {
      sid       = "AllowKmsDecrypt${replace(var.bucket_name, "-", "")}"
      effect    = "Allow"
      actions   = ["kms:Decrypt"]
  ...
  statement {
    sid       = "AllowObjectRead${replace(var.bucket_name, "-", "")}"
    effect    = "Allow"
...
data "aws_iam_policy_document" "bucket_write_policy" {
  source_policy_documents = [data.aws_iam_policy_document.bucket_read_policy.json]
  statement {
    sid    = "AllowObjectAdmin${replace(var.bucket_name, "-", "")}"
    effect = "Allow"
    actions = [
      "s3:PutObject*",
      "s3:DeleteObject*",
  ...
  dynamic "statement" {
    for_each = var.special_encrypted ? [] : ["allow_to_use_kms_key"]
    content {
      sid    = "AllowKmsAccess${replace(var.bucket_name, "-", "")}"
      effect = "Allow"
      actions = [
        "kms:Encrypt",
        "kms:ReEncrypt*",
